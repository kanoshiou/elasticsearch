...
public class WildcardPatternList extends AbstractStringPattern implements Writeable {
     ...
     */
    @Override
    protected Automaton doCreateAutomaton(boolean ignoreCase, CircuitBreaker breaker) {
        List<Automaton> automatonList = patternList.stream().map(x -> x.createAutomaton(ignoreCase, breaker)).toList();
        Automaton result = Operations.union(automatonList);
        return breaker != null
            ? CircuitBreakingOperations.determinize(result, Operations.DEFAULT_DETERMINIZE_WORK_LIMIT, breaker, "esql-like-rlike")
            : Operations.determinize(result, Operations.DEFAULT_DETERMINIZE_WORK_LIMIT);
    }
